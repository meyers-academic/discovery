...
def sample_rhos(key, psrs, params, cs, invhdorf=None):
    ...
    for psr in psrs:
        model_names_non_gw.extend([k.split(psr.name)[1].split("log10_rho")[0][1:-1] for k in params if psr.name in k and 'gw' not in k and 'log10_rho' in k])
    ...
    if not set(np.unique(model_names_non_gw)) == set(np.unique(coeff_model_names_non_gw)):
        print(set(np.unique(model_names_non_gw)), set(np.unique(coeff_model_names_non_gw)))
...
def setup_hmc_gibbs(ll_object, psrs, priordict=ds.priordict_standard,  invhdorf=None, nuts_kwargs={}):
    jlogl = jax.jit(ll_object.clogL)
    sample_cond = ll_object.sample_conditional
    jsc = jax.jit(sample_cond)
    parnames = ll_object.clogL.params
    gibbs_params = [k for k in parnames if 'log10_rho' in k or 'coefficients' in k]
    ...
    def gibbs_step(rng_key, gibbs_sites, hmc_sites):
        ...
        # for key, val in cs.items():
            # numpyro.sample(key, dist.deterministic(key, val)
