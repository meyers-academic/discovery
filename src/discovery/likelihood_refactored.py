...
from . import matrix_refactored as matrix
from . import signals

# This is the refactored version of likelihood.py that uses the unified
# WoodburyKernel classes from matrix_refactored.py instead of the original
# matrix.py implementations.
    ...
    def __init__(self, args, concat=True):
        ...
        if cgps:
            if len(cgps) > 1 and concat:
                ...
                csm = matrix.WoodburyKernel(noise, cgp.F, cgp.Phi, refactored=True)
            else:
                csm = noise
                for cgp in cgps:
                    csm = matrix.WoodburyKernel(csm, cgp.F, cgp.Phi, refactored=True)
        else:
            csm = noise
        ...
        if vgps:
            ...
            if len(vgps) > 1 and concat:
                ...
                vsm = matrix.WoodburyKernel(csm, vgp.F, vgp.Phi, refactored=True)
                vsm.index = getattr(vgp, 'index', None)
                vsm.mean = getattr(vgp, 'mean', None)
            else:
                vsm = csm
                for vgp in vgps:
                    vsm = matrix.WoodburyKernel(vsm, vgp.F, vgp.Phi, refactored=True)
                    vsm.index = getattr(vgp, 'index', None)
                    vsm.mean = getattr(vgp, 'mean', None)
        else:
            vsm = csm
            ...
            if rank == 0:
                ...
                def loglike(params):
                    ...
                    if kmeans is not None:
                        ...
                        a0 = kmeans(params)
                        FtNmFa0 = FtNmF @ a0
                        ret = ret - (0.5 * FtNmFa0.T - FtNmy.T) @ (a0 - matrix.jsp.linalg.cho_solve(cf, FtNmFa0))
